...
class StateProps:
    """
    A simple class to hold the thermodynamic state properties.
    """
    def __init__(self):
        self.p = None  # Pressure in bar
        self.T = None  # Temperature in Celsius
        self.h = None  # Enthalpy in kJ/kg
        self.s = None  # Entropy in kJ/kg.K
        self.v = None  # Specific volume in m^3/kg
        self.x = None  # Quality (None if not two-phase)

    def update(self, p=None, T=None, x=None):
        """
        Update the state properties using pyXsteam based on provided parameters.
        Either T (temperature) or x (quality) needs to be provided along with pressure p.
        """
        if T is not None:
            self.T = T
            self.h = steamTable.h_pt(p, T)
            self.s = steamTable.s_pt(p, T)
            self.v = 1 / steamTable.rho_pt(p, T)  # Specific volume = 1 / density
        elif x is not None:
            self.x = x
            ...
            if x == 0:
                ...
                self.v = 1 / steamTable.rhoL_p(p)
            elif x == 1:
                self.h = steamTable.hV_p(p)
                self.s = steamTable.sV_p(p)
                self.v = 1 / steamTable.rhoV_p(p)
            else:
                self.h = steamTable.h_px(p, x)
                self.s = steamTable.s_px(p, x)
                self.v = x / steamTable.rhoV_p(p) + (1 - x) / steamTable.rhoL_p(p)
...
class rankineController():
    def __init__(self, *args):
        ...
        if args:
            ...
            self.IW = []  # Initialize as empty or suitable defaults
            self.DW = []  # Initialize as empty or suitable defaults
    ...
    def calc_efficiency(self):
        ...
        self.calculate_state_properties()  # Ensure all state properties are updated
...
def main():
    ...
    print(eff)
    RC.print_summary()
    RC.plot_cycle_TS()  # This now correctly calls the method via the controller



    state1 = StateProps()
    state1.update(p=80, T=500)  # Superheated steam at turbine inlet

    state2 = StateProps()
    state2.update(p=0.08, x=0)  # Saturated liquid at condenser inlet

    # Continue defining states and implementing the Rankine cycle logic
    print(f"State 1: h={state1.h} kJ/kg, s={state1.s} kJ/kg.K")
    print(f"State 2: h={state2.h} kJ/kg, s={state2.s} kJ/kg.K")

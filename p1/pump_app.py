...
class PumpCurve_GUI_Class(Ui_Form, qtw.QWidget):  #class for PumpCurve_GUI inherits from Ui_Form & qtw.QWidget
    def __init__(self): #constructor for this class
        ...
        self.myPump=Pump_Controller()  # Instantiates the Pump_Controller
    ...
    def OpenFile(self):
        ...
        fname, _ = qtw.QFileDialog.getOpenFileName(self, "Open File", self.FilePath, "All Files (*);;Text Files (*.txt)", options=options)
        oTF = len(fname) > 0
        if oTF:
            self.FileName = fname
